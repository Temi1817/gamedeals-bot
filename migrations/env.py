"""Alembic environment: async-движок, URL и метаданные берём из приложения."""

from __future__ import annotations

import asyncio
import sys
from logging.config import fileConfig
from pathlib import Path

from alembic import context
from sqlalchemy import pool
from sqlalchemy.engine import Connection
from sqlalchemy.ext.asyncio import async_engine_from_config

# alembic запускается из корня проекта, но подстрахуемся
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

from bot.config import ROOT_DIR, database_url_from_env
from bot.db.models import Base

config = context.config

# fileConfig перенастраивает логирование целиком по alembic.ini, где root
# стоит на WARN. Когда миграции гоняет сам бот, это глушит все его INFO-логи,
# поэтому там мы выставляем configure_logger=False.
if config.config_file_name is not None and config.attributes.get(
    "configure_logger", True
):
    fileConfig(config.config_file_name)


def _resolved_url() -> str:
    """URL из окружения; относительный путь SQLite делаем абсолютным, чтобы
    alembic из любой директории бил в ту же базу, что и бот."""
    url = database_url_from_env()
    prefix = "sqlite+aiosqlite:///"
    if url.startswith(prefix):
        raw = url[len(prefix) :]
        if raw and not raw.startswith(":memory:") and not Path(raw).is_absolute():
            path = ROOT_DIR / raw
            path.parent.mkdir(parents=True, exist_ok=True)
            url = prefix + path.as_posix()
    return url


config.set_main_option("sqlalchemy.url", _resolved_url())

target_metadata = Base.metadata


def _configure(**kwargs: object) -> None:
    context.configure(
        target_metadata=target_metadata,
        compare_type=True,
        compare_server_default=True,
        # SQLite не умеет ALTER COLUMN — batch-режим пересоздаёт таблицу
        render_as_batch=True,
        **kwargs,  # type: ignore[arg-type]
    )


def run_migrations_offline() -> None:
    _configure(
        url=config.get_main_option("sqlalchemy.url"),
        literal_binds=True,
        dialect_opts={"paramstyle": "named"},
    )
    with context.begin_transaction():
        context.run_migrations()


def do_run_migrations(connection: Connection) -> None:
    _configure(connection=connection)
    with context.begin_transaction():
        context.run_migrations()


async def run_async_migrations() -> None:
    connectable = async_engine_from_config(
        config.get_section(config.config_ini_section, {}),
        prefix="sqlalchemy.",
        poolclass=pool.NullPool,
    )
    async with connectable.connect() as connection:
        await connection.run_sync(do_run_migrations)
    await connectable.dispose()


def run_migrations_online() -> None:
    asyncio.run(run_async_migrations())


if context.is_offline_mode():
    run_migrations_offline()
else:
    run_migrations_online()
